...
def search_manga(
	...
	Args:
		...
		title (str, optional): Name of the title to search. For it to work, the only parameter set must be the title.
		...
		include_mode (str, optional): Valid options: "and" (all selected genres) and "or" (any selected genre).

		chapters (string, optional): Minimum chapters numbers.


		statuts (str, optional): Current manga status. Valid options: "0" (cancelled), "1" (ongoing), "2" (completed).
	...
	"shounen", "shounen-ai", "slice-of-life", "smut", "sports", "super-power", "supernatural", "survival", "time-travel",
	...
	ex_genres = "exclude=" + "_".join(exclude_genres)

	# Append to base URL
	url += f"filter=1&order={sort_by}&include_mode={include_mode}&chapters={chapters}&status={status}&{genres}&{ex_genres}"
...
if __name__ == "__main__":
	# print(get_manga_data("http://mangakatana.com/manga/attack-on-titan.842")["chapters_data"]["chapter_1"])
	# lista = get_chapter_images_links("https://mangakatana.com/manga/attack-on-titan.842/c130")
	# download_chapter_images(lista, "/home/nunnito/")
	# covers_list = []
	# links_list = []
	# populars = get_manga_populars()
	# for i in populars:
		# covers_list.append(i["manga_cover"])
		# links_list.append(i["manga_link"])
	# download_and_save_manga_covers(covers_list, links_list)
	print(search_manga(sort_by="az", chapters="10", page="10", status="1", include_mode="or", genres=["seinen", "ecchi"], exclude_genres=["yaoi", "josei"]))
