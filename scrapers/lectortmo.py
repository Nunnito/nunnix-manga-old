...
def get_manga_data(url):
	...
	try:
		manga_data["total_chapters"] = len(page_source.find(
			id="chapters").find_all("h4", {"class": "px-2 py-3 m-0"}))
	except AttributeError:
	...
	if len(chapters_data) == 0:
		...
		for chapter_data in chapters_data:
			...
			chapters_fansub.append(chapter_data.find_all("div", {"class": "card chapter-list-element"})[0].find(
				"div", {"class": "text-truncate"}).text.strip())
...
def download_chapter_images_to_pdf(image_list_url, dir_, name):
	...
	with TemporaryDirectory() as temp_dir:
		for download_image in range(len(image_list_url)):
			# Adds the manga image data to "mistermanga".
			# For higher download speed

			# image_page = "https://sv.mistermanga.com/uploads/" + "/".join(image_list_url[download_image].split("/")[-2:])
...
def search_manga(
	...
	Args:
		...
		erotic (str, optional):
			...
			If not specified, the erotic content will be combined with the others. Valid options: "true", "false".
		...
		webcomic (str, optional):
			...
			If not specified, the webcomic content will be combined with the others. Valid options: "true", "false".

		yonkoma (str, optional):
			Show only yonkoma content, or don't show yonkoma content.
			If not specified, the yonkoma content will be combined with the others. Valid options: "true", "false".

		amateur (str, options):
				Show only amateur content, or don't show amateur content.
			If not specified, the amateur content will be combined with the others. Valid options: "true", "false".
	...
	if genre_id == [] and exclude_genres_id == []:
		# Append to base URL
		...
		genres = "".join(["&genders[]=" + genre for genre in genre_id])
...
# 	form_filters = page_source.find(id="form-filters")
# 	print(form_filters)
...
def download_and_save_manga_covers(covers_links, manga_links, max_covers=15, threads=5):
	...
	for number in range(len(manga_links)):
		...
		# If "number" equals "aux", wait until the current threads have finished
		if number == aux:
...
def download_image_cover(cover_link, manga_link):
	...
	while True:
		try:
			...
			downloaded_image = requests.get(cover_link, timeout=5, headers=headers).content
		...
		# If when downloading the image, it gives an error
		except (ReadTimeout, ConnectionError):
			pass


if __name__ == "__main__":
	# print(get_manga_data("https://lectortmo.com/library/manga/39964/isekai-shoukan-wa-nidome-desu"))
	# print(get_manga_popular())
	print(search_manga(exclude_genres_id=["6", "19"]))
